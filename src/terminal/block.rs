        ...
        // Live command filter: typing narrows the block list by command substring.
        {
            ...
            filter_entry.connect_search_changed(move |e| {
                *ctx.filter_query.borrow_mut() = e.text().to_string();
                ...
                // Without this, the minimap tick count stays frozen on the
                // pre-filter set while the user types — the ticks visibly lie
                // about how many blocks survived the filter.
                rebuild_minimap(&ctx);
            });
        }
        {
...
fn handle_event(ctx: &Rc<Ctx>, sender: &ComponentSender<BlockTerminal>, ev: ParserEvent) {
    match ev {
        ...
        // be answered here. Add a new arm below whenever the parser starts
        // recognizing another query (kitty graphics responses, DECRQSS, etc.).
...
/// Format an XParseColor reply for an OSC 10/11/12/4 query. The active VTE
/// would normally emit this itself; in block mode we synthesize it from the
/// configured palette and write it back to the PTY so the requesting app
/// (neovim, helix, anything probing dark-vs-light background) sees a response.
fn build_color_query_reply(ctx: &Rc<Ctx>, kind: ColorKind) -> String {
    use gtk4::gdk::RGBA;
    let config = ctx.config.borrow();
    let rgba: RGBA = match kind {
        ...
        ColorKind::Palette(idx) => {
            // 16 named palette entries; 16-255 derived from the xterm cube +
            // grayscale ramp (mirrors `ansi::ansi256_to_rgb`).
            let (r, g, b) = super::ansi::ansi256_to_rgb(idx, &config.palette);
